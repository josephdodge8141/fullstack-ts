...
function config(): AwsPreviewConfig {
  ...
  const routerImage = optional('PREVIEW_ROUTER_IMAGE');
  const composeFile = optional('PREVIEW_COMPOSE_FILE');
  return {
    ...
    ...(composeFile === undefined
      ? {}
      : { compose: compileNormalizedCompose(JSON.parse(readFileSync(composeFile, 'utf8'))) }),
    ...(routerImage === undefined ? {} : { routerImage }),
  ...
  if (operation === 'sweep') {
    ...
    for await (const state of stateStore.list()) {
      try {
        ...
        if (due === null && state.active?.phase === 'launching') continue;
        if (
          due === null &&
          state.active?.phase !== 'cleaning' &&
          state.retiring?.phase !== 'cleaning'
        )
          continue;
    ...
    if (failures.length > 0)
      throw new AggregateError(failures, 'preview sweep left cleanup failures');

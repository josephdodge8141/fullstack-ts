...
const receipt = {
  ...
  taskArn: 'task',
  taskDefinitionArn:
    'arn:aws:ecs:us-east-1:111111111111:task-definition/preview-pr-7-preview-123-7-1:1',
...
test('partial launch receipt is retained for cleanup when registration succeeded but task never started', async () => {
  const commands: unknown[] = [];
  const partial = { ...receipt, taskArn: '', publicIp: '', status: 'launching' as const };
  const provider = providerFor(
    async (command) => {
      commands.push(command);
      if (command instanceof GetCommand) return { Item: { previewKey: 'receipt', ...partial } };
      return {};
    },
    async (command) => {
      commands.push(command);
  ...
  await provider.cleanupPreview({ type: 'cleanup-preview', ownership, reason: 'startup-timeout' });
  assert.equal(
    commands.some((command) => command instanceof DescribeTasksCommand),
    false,
  );
  assert.equal(
    commands.some((command) => command instanceof ChangeResourceRecordSetsCommand),
    false,
  );
  assert.equal(
    commands.some((command) => command instanceof DeregisterTaskDefinitionCommand),
    true,
  );
  assert.equal(
    commands.some((command) => command instanceof UpdateCommand),
    true,
  );
});

test('cleanup failure leaves receipt retryable until every effect succeeds', async () => {
  const commands: unknown[] = [];
  let failDns = true;
  const provider = providerFor(
    async (command) => {
      commands.push(command);
      if (command instanceof GetCommand) return { Item: { previewKey: 'receipt', ...receipt } };
      return {};
    },
    async (command) => {
      commands.push(command);
    ...
    async (command) => {
      commands.push(command);
      if (failDns) throw new Error('Route 53 temporarily unavailable');
      return {};
    },
  );
  const effect: Extract<LifecycleEffect, { type: 'cleanup-preview' }> = {
  ...
  await assert.rejects(() => provider.cleanupPreview(effect), /temporarily unavailable/);
  assert.equal(
    commands.some((command) => command instanceof UpdateCommand),
    false,
  );
...
test('incomplete receipt discovers only its tagged task definition for cleanup', async () => {
  const commands: unknown[] = [];
  const definition = receipt.taskDefinitionArn;
  const partial = {
    ...receipt,
    taskArn: '',
    taskDefinitionArn: '',
  ...
  const provider = providerFor(
    ...
    async (command) => {
      ...
      if (command instanceof ListTaskDefinitionsCommand)
        return { taskDefinitionArns: [definition] };
      if (command instanceof ListTagsForResourceCommand)
        return {
          tags: [
            { key: 'fullstack:repository-id', value: ownership.repositoryId },
            { key: 'fullstack:pull-request', value: String(ownership.pullRequestNumber) },
            { key: 'fullstack:generation', value: ownership.generation },
          ],
        };
      return {};
    },
  ...
  await provider.cleanupPreview({ type: 'cleanup-preview', ownership, reason: 'startup-timeout' });
  assert.equal(
    commands.some((command) => command instanceof ListTaskDefinitionsCommand),
    true,
  );
  assert.equal(
    commands.some((command) => command instanceof DeregisterTaskDefinitionCommand),
...
test('incomplete receipt refuses a discovered definition with foreign ownership tags', async () => {
  const partial = {
    ...receipt,
    taskArn: '',
    taskDefinitionArn: '',
  ...
  const provider = providerFor(
    async (command) =>
      command instanceof GetCommand ? { Item: { previewKey: 'receipt', ...partial } } : {},
    async (command) => {
      if (command instanceof ListTasksCommand) return { taskArns: [] };
      if (command instanceof ListTaskDefinitionsCommand)
        return { taskDefinitionArns: [receipt.taskDefinitionArn] };
      if (command instanceof ListTagsForResourceCommand)
        return { tags: [{ key: 'fullstack:generation', value: 'another-generation' }] };
      if (command instanceof DeregisterTaskDefinitionCommand) deregistered = true;
      return {};
    },
    async () => ({}),
  );
  await assert.rejects(
    () =>
      provider.cleanupPreview({ type: 'cleanup-preview', ownership, reason: 'startup-timeout' }),
    /ownership tags/,
  );
  assert.equal(deregistered, false);
});

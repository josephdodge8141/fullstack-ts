...
Frontend pages own top-level layouts and local behavior. Services perform API transport and schema validation only. Shared components, context and hooks require the exact reuse rule. The default starter has no authentication. Playwright supports development and CI; deployed preview verification runs canonical Cucumber and Playwright. The independent browser agent is optional.
